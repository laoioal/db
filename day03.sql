...
SELECT
    FLOOR(SYSDATE - TO_DATE('94/12/26')) "살아온 날수", -- * 3 as "밥그릇 수"
    FLOOR(SYSDATE - TO_DATE('94_12 26')) 날수
FROM
    dual
;
...
SELECT
    FLOOR(SysDATe - TO_DATE('12/26/1994', 'MM/DD/YYYY')) 산날
FROM
    dual
;
...
SELECT
    FLOOR(SYSDATE - TO_DATE('12261994', 'MMDDYYYY')) 산날
FROM
    dual
;
...
SELECT
    '123' + '456' 결과    -- 이 경우는 형변환 함수가 자동 호출이 되서 계산이 된다.
FROM
    dual
;
...
SELECT
    TO_NUMBER('123') + TO_NUMBER('456') 결과
FROM
    dual
;
...
SELECT
    ename 사원이름, sal 급여, comm 커미션, COALESCE(sal * 1.1 + comm, sal * 1.05 + 100) 지급급여
FROM
    emp
;
/*
...
SELECT
    ename 사원이름, sal 급여, comm 커미션, COALESCE(comm * 1.5, sal * 1.1 ) 지급커미션
FROM
    emp
;
...
SELECT
   ...
   DECODE(deptno, 10, '회계부',
                    ...
                    30, '영업부',
                    '관리부') 부서이름,
    MAX(sal) 부서최대급여, FLOOR(AVG(sal)) 급여평균
