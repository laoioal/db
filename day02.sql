...
/*
    참고 ]
        오라클이 데이터를 보관하는 방법
            ...
            참고 ]
                ERD - Entity Relation Diagram
                        테이블들 간의 관계를 도식화 한 다이어그램
                        
            참고 ]
                오라클은 테이블들 간의 관계를 형성해서 데이터를 저장한다.
                이런 종류의 데이터베이스 관리 시스템을
                    RDBMS - 개체들간의 관계를 형성해서 데이터를 관리하는 시스템
                            관계형데이터베이스 관리 시스템
                            
            참고 ]
                정형데이터베이스
                     7934	MILLER	CLERK	7782	82/01/23	1300		10
                     사원번호   이름  직급      널   입사일     급여  널   널
                            
                                분산저장 분산처리가 안된다.
                    
                비정형데이터베이스
                    형태가 갖춰지지 않아도 저장할 수 있는  데이터베이스
                    NoSQL
                        MongoDB
        ...
        참고 ]
            ...
            이떄 확보된 메모리 공간은 서로 공유가 안된다.
            
            따라서 접속한 사람은 확보한 공간에서만 작업(DML 명령)을 하게되고
            최종적으로 데이터베이스에 적용시키는 작업(DCL 명령)은 별도로 명령을 해야한다.(TCL 명령)
         
*/

-- 내가 접속한 계정안에 테이블들 이름들을 조회해보자.
...
/*
    참고 ]
        ...
        필드 이름을 나열하는 대신 ' * ' 기호를 사용해서 대신 하는 경우가 있다.
...
-- 사원들의 연봉을 계산해서 사원이름, 입사일, 연봉을 조회하세요. 연봉은 급여 * 12 + 커미션, 커미션이 없는 사원은 0으로 계산하세요
SELECT
...
ORDER BY
    부서번호 ASC, 급여 DESC
    -- 부서번호 ASC, 급여 DESC -- 조회된 결과를 가지고 정렬하기 때문에 같은 의미이다.
;
...
/*
    ...
    ==> 데이터를 가공하기 위해 오라클이 제시한 명령들 또는 개체
        ...
        오라클 함수 종류 ]
            ...
            1. 단일행 함수
                ==> 한줄 한줄마다 매번 명령이 실해되는 함수
                    따라서 단일행 함수의 결과는 출력되는 데이터의 개수와 동일하다.
            
            2. 그룹 함수
                ==> 여러행이 모여서 한번만 실행되는 함수
                    따라서 그룹 함수는 출력개수가 오직 한개인다.
                    집계함수들이 그룹함수에 해당한다.
...
*/
SELECT
    LPAD(SUBSTR(ename, -2), length(ename), '#'), hiredate, sal
FROM
	emp
;
...
*/
SELECT
CONCAT(CONCAT(CONCAT(LPAD(SUBSTR('jennie@githrd.com', 3, 1), INSTR('jennie@githrd.com', SUBSTR('jennie@githrd.com', 3, 1)) , '*'),
SUBSTR(RPAD(SUBSTR('jennie@githrd.com', 3, 1), (instr('jennie@githrd.com', '@') - INSTR('jennie@githrd.com', SUBSTR('jennie@githrd.com', 3, 1))) , '*'), 2, 4)
